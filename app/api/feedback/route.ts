...
export async function POST(req: NextRequest) {
  try {
    ...
    // Save to MongoDB
    await connectDB();
    await Feedback.create({ name, email, phone, message, rating: rating || undefined });

    // Send email via Nodemailer (wrapped in separate try-catch so it won't crash DB inserts)
    try {
      ...
      const stars = rating ? '★'.repeat(rating) + '☆'.repeat(5 - rating) : 'None';
      ...
      console.error('Nodemailer failed to send feedback email:', emailErr);
    }
